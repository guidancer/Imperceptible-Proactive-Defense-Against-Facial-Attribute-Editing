...
from utils.loss_balance import normal_loss_balance as loss_scale
from torch.utils.data import DataLoader
from sfgan.sfnet import SFGANNet
from utils.celeba_dataset import AttributeDataCelebA as CelebA
from models.fgan_model import FModel
from models.attgan_model import AttModel
from models.stargan_model import StarModel
from models.attentiongan_model import AttentionModel
...
if __name__ == '__main__':
    ...
    config_parser.add_argument('--batch_size', default=2, type=int, help='Batch Size For Training')
    ...
    model_save_path = os.path.join('./sfgan_weights', 'ensemble')
    ...
    """Set Deepfake Models As White Box"""
    fgan_model = FModel(train_dataloader)
    fgan_model.eval().to(device)
    attgan_model = AttModel(attgan_args)
    attgan_model.eval().to(device)
    attentiongan_model = AttentionModel(train_dataloader,args_attack.AttentionGAN)
    attentiongan_model.eval().to(device)
    
    """Set Deepfake Models As Black Box"""
    stargan_model = StarModel(train_dataloader,args_attack.stargan)
    stargan_model.eval().to(device)
    ...
    for epoch in range(1, n_epochs):  # outer loop for different epochs;
        ...
        for idx, (img_a, att_a, c_org) in enumerate(tqdm(train_dataloader, desc='')):  # inner loop within one epoch
            ...
            """Get Labels For Others And AttGAN"""
            c_org_cuda = c_org.to(device)
            ...
            """Forward It"""
            """Get the Original FGAN result"""
            f_results_ori = fgan_model.f_modify(img_a_cuda,c_org_cuda)
            """Get the Adversarial FGAN result"""
            f_results_adv = fgan_model.f_modify(adv_img_a,c_org_cuda)
            """Get the Original StarGAN result"""
            attention_results_ori = attentiongan_model.attention_modify(img_a_cuda,c_org_cuda)
            """Get the Adversarial StarGAN result"""
            attention_results_adv = attentiongan_model.attention_modify(adv_img_a,c_org_cuda)
            """Get the Original AttGAN result"""
            att_results_ori = attgan_model.att_modify(img_a_cuda,att_a_cuda)
            """Get the Adversarial AttGAN result"""
            ...
            pred_adv_real = sfgannet.netD(adv_img_a)
            ...
            loss_attgan_adv = 0
            loss_fgan_adv = 0
            loss_attentiongan_adv = 0
            for i in range(num_attrs):
                loss_fgan_adv += mse_loss(f_results_ori[i]*sod_face_mask, f_results_adv[i]*sod_face_mask)
                loss_attgan_adv += mse_loss(att_results_ori[i]*sod_face_mask, att_results_adv[i]*sod_face_mask)
                loss_attentiongan_adv += mse_loss(attention_results_ori[i]*sod_face_mask, attention_results_adv[i]*sod_face_mask)
        ...
        print('epoch %d, loss G is %f, loss adv is %f, loss visual is %f, loss gan is %f'%(epoch, train_current_loss['full_G'],train_current_loss['adv'],train_current_loss['visual'],train_current_loss['gan']))
        """Training Chapter End"""

        save_filename_model = 'ensemble_no_star_sfgannet_%s_%s_epoch%s_epsilon%s.pth'%(lambda_visual, lambda_gan, epoch, epsilon)
