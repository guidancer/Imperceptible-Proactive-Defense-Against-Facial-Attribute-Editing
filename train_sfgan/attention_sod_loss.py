...
if __name__ == '__main__':
    ...
    model_save_path = os.path.join('./sfgan_weights', 'attentiongan')
    ...
    train_dataset = CelebA(args_attack.global_settings.data_path, args_attack.global_settings.attr_path, args_attack.global_settings.img_size, 'train', attgan_args.attrs,args_attack.AttentionGAN.selected_attrs)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, num_workers=16,shuffle=True, drop_last=False)
    ...
    for epoch in range(1, n_epochs):  # outer loop for different epochs;
        ...
        for idx, (img_a, att_a, c_org) in enumerate(tqdm(train_dataloader, desc='')):  # inner loop within one epoch
            ...
            for i in range(num_attrs):
                loss_adv += mse_loss(attention_results_ori[i]*sod_face_mask, attention_results_adv[i]*sod_face_mask)
            loss_adv /= num_attrs
        ...
        save_filename_model = 'sfgannet_sod_loss_attentiongan_%s_%s_epsilon_%s_epoch%s.pth'%(lambda_visual, lambda_gan, epsilon, epoch)
