...
if __name__ == '__main__':
    ...
    model_save_path = os.path.join('./sfgan_weights', 'fgan')
    ...
    for epoch in range(1, n_epochs):  # outer loop for different epochs;
        ...
        for idx, (img_a, att_a, c_org) in enumerate(tqdm(train_dataloader, desc='')):  # inner loop within one epoch
            ...
            fgan_results_ori = fgan_model.f_modify(img_a_cuda,c_org_cuda)
            """Get the Adversarial StarGAN result"""
            fgan_results_adv = fgan_model.f_modify(adv_img_a,c_org_cuda)
            ...
            for i in range(num_attrs):
                loss_adv += mse_loss(fgan_results_ori[i]*sod_face_mask, fgan_results_adv[i]*sod_face_mask)
            loss_adv /= num_attrs
        ...
        save_filename_model = 'sfgannet_sod_loss_fgan_%s_%s_epoch%s_epsilon%s.pth'%(lambda_visual, lambda_gan, epoch, epsilon)
